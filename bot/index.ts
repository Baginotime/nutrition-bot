...
bot.start(async (ctx) => {
  try {
    ...
    // Показываем клавиатуру с кнопками меню
    await ctx.reply(
...
bot.hears('📋 Меню', async (ctx) => {
  await ctx.reply('🍽️ Раздел меню будет доступен скоро!', menuKeyboard);
});

bot.hears('📊 Отчеты', async (ctx) => {
  await ctx.reply('📈 Раздел отчетов будет доступен скоро!', menuKeyboard);
});

bot.hears('⚙️ Настройки', async (ctx) => {
  await ctx.reply('🔧 Раздел настроек будет доступен скоро!', menuKeyboard);
});
...
bot.hears('ℹ️ Помощь', async (ctx) => {
  await ctx.reply(
    'ℹ️ Помощь\n\n' +
    'Используй кнопки меню для навигации.\n' +
    'Для заполнения анкеты нажми "📝 Заполнить анкету"',
    menuKeyboard
  );
});
...
bot.hears('📝 Заполнить анкету', async (ctx) => {
  await ctx.reply(
    '📝 Заполни анкету для расчета твоей нормы калорий!',
    {
      reply_markup: {
        inline_keyboard: [
          [
...
bot.on('message', async (ctx) => {
  ...
  await ctx.reply(
    'Не понял. Используй кнопки меню или напиши /start для начала работы.',
    menuKeyboard
  );
});
